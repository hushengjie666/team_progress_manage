...
func TestMySQLWorkspaceSwitchingPreservesGlobalDataAndSharedMemberships(t *testing.T) {
	...
	saveRows(t, api, privateAuth, "device_private", []businessRow{{
		Entity:    "project",
		ID:        "project_private",
	...
	saveRows(t, api, sharedAuth, "device_shared", []businessRow{{
		Entity:    "project",
		ID:        "project_shared",
	...
	switchBody := versionedJSONBody(t, `{"workspace_id":"`+privateLogin.Workspace.ID+`","device_id":"device_shared"}`, 0)
	...
	deniedSwitchBody := versionedJSONBody(t, `{"workspace_id":"`+privateLogin.Workspace.ID+`","device_id":"device_member"}`, 0)
	...
	staleRequest := httptest.NewRequest(http.MethodGet, "/team/data", nil)
	staleRequest.Header.Set("Authorization", "Bearer "+staleToken)
	staleRecorder := httptest.NewRecorder()
	api.withAuth(api.handleTeamDataLoad)(staleRecorder, staleRequest)
