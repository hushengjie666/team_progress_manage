...
func TestMySQLProjectInvitationAcceptAddsProjectMembershipOnly(t *testing.T) {
	...
	api.handleTeamDataLoad(stateRecorder, httptest.NewRequest(http.MethodGet, "/team/data", nil), inviteeAuth)
...
func TestMySQLProjectInvitationCancelHidesPendingInvitation(t *testing.T) {
	...
	sharedAuth := authContext{AccountID: adminLogin.Account.ID, WorkspaceID: sharedLogin.Workspace.ID}
	workspaceID := sharedLogin.Workspace.ID
	saveRows(t, api, sharedAuth, "device_seed_cancel", []businessRow{
