...
if [[ ! -f "${CONFIG_DIR}/sync.json" ]]; then
  ...
  echo "Edit ${CONFIG_DIR}/sync.json and set mysql_dsn before starting the service."
  CREATED_CONFIG=1
fi
...
if [[ "${CREATED_CONFIG}" -eq 0 ]]; then
  ...
  echo "Service installed but not started. Edit ${CONFIG_DIR}/sync.json, then run: sudo systemctl start timemanage-sync"
fi
