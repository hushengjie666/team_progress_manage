...
import { createWorkspace, loginToWorkspace, switchWorkspace, updateWorkspace } from "./sync";
...
describe("workspace sync api", () => {
  it("maps multi-workspace login responses for workspace switching", async () => {
    ...
    const session = await loginToWorkspace(sync, "owner@example.com", "secret");
  ...
  it("posts workspace switching and workspace creation requests to the backend", async () => {
    ...
    await switchWorkspace(sync, "token_current", "workspace_next");
    await createWorkspace(sync, "token_next", "新协作区");
